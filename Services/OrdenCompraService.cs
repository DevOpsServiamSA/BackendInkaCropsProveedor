...
public class OrdenCompraService : _BaseService
{
    ...
    public async Task<object> GetAllSPAsync(DateTime p_fi, DateTime p_ff, int p_estado,
                                            ...
                                            string? p_prov_ruc = null)
    {
        try
        {
            var result = await _context.OrdenCompraResponse.FromSqlInterpolated($"exec oc_get_orden_compra {p_fi.Date}, {p_ff.Date}, {p_fid.Date}, {p_ffd.Date}, {p_estado}, {p_prov_rs}, {p_prov_ruc}").ToListAsync();
